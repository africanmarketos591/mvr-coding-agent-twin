...
8. **REQUIRED before completion or delivery wording:** `python <TWIN_DIR>/scripts/twin_verify_run.py --root . --stage export --keyfile <keyfile> --write-status`. Lead the final response with the exact banner in `mvr/final-status.json`. No key/offline is exit 3 (`inconclusive`), never a pass. Exit 0 authenticates kernel authority and the governed, hash-bound build surface; it does **not** replace dependency installation, app tests, security review, or production-readiness checks. Every required product command must also exit 0.
...
- `scripts/twin_verify_run.py` — stage-aware governance-evidence audit. It reports live authority, material-claim coverage, per-capability disposition, authorization and claim-surface consistency, export authorization, semantic compliance, export artifacts, and runtime assurance separately. `--write-status` emits the mandatory final-response banner. Export requires an explicit pilot-or-higher decision ceiling. It refuses to equate an arbitrary offline 64-hex string with provenance; runtime assurance remains `not_evaluated` until separate app, security, accounting, and authentication tests run.
- `scripts/twin_audit_pack.py` — deterministic reviewer handoff built from the actual decision log or seed and exact semantic-review request. It records command exits, hashes included files, can include product carriers, and excludes common secret/private files.
...
- `tests/test_twin_verify_run.py` — forged-shaped, fake-hash, outage, live-authority, build, export, final-banner, and live-receipt/failed-product-tripwire evidence states.
- `tests/test_twin_audit_pack.py` — seed preservation, exact target derivation, failure retention, and product-carrier packaging.
