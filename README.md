...
- **Product-surface fabrication scan:** `scripts/twin_fabrication_scan.py` runs before export and flags unhedged licence numbers, licensed partners, hard fee/capital/cover figures, and unauthorized regulated capabilities in demos/plans/source files that are not backed by current-format evidence or local Twin authorization. With no `--targets`, it scans root product docs and common app folders such as `src`, `app`, `frontend`, `backend`, `scaffold`, and `*-app`.
