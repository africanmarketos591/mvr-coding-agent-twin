...
- **PATH-EVASION HONESTY.** The claim gate pattern-matches `claims/`. Writing an investor deck under `docs/` to dodge the gate is possible, forbidden (Law 5's spirit), and self-defeating: an exported claim without a gate receipt and kernel verification fails the external check above. If you catch yourself drafting claim-shaped content outside `claims/`, that is the drift countermeasure firing — move it and run PRE-CLAIM.
