...
4. Build only the fitted wedge. Run `python scripts/twin_build_spec.py --root . --review-request src`; review the exact files semantically against every forbidden constraint and write `mvr/semantic-review.json` using the request schema.
5. Before commit/export, run `python scripts/twin_build_spec.py --root . --check src --require-semantic-review`. A clear lexical tripwire alone is not semantic assurance.
