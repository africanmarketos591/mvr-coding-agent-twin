...
10. Audit Twin evidence: `python <TWIN_DIR>/scripts/twin_verify_run.py --root . --stage export --keyfile <key>`. Export requires an explicit pilot-or-higher decision authorization; reviews cannot create that authority. Only exit `0` verifies live authority plus the governed build surface. Read every reported dimension separately: authority, material-claim coverage, authorization consistency, export authorization, semantic compliance, export artifacts, and runtime assurance. Runtime remains `not_evaluated` until separate product checks run. Exit `3` is inconclusive, not a pass.
