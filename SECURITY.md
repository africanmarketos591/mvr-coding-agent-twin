...
- `scripts/twin_verify_run.py` loads an optional key file with the strict key parser, restores the prior environment value, and never prints the key. Its exit 0 means a live authority hash verified and the requested stage's local artifacts are internally consistent. It does not cryptographically prove which host authored every file; without a key or during outage it returns inconclusive, never verified.
