...
- `scripts/twin_verify_run.py` loads an optional key file with the strict key parser, restores the prior environment value, and never prints the key. Its exit 0 means a live authority hash verified and the requested stage's governed, hash-bound build surface is internally consistent and tripwire-clear. It does not cryptographically prove which host authored every file or certify app runtime, dependencies, security, demand, or production readiness; without a key or during outage it returns inconclusive, never verified.
